...
interface DisplayPersonaFacultadFromList extends ApiPersonaFacultad {
  id: string;
  displayName: string;
  nombres?: string;
  apellidos?: string;
...
export default function PersonaFacultadFormScreen() {
  const styles = getStyles("light");
  const router = useRouter();
  const params = useLocalSearchParams<{
    personaFacultadId?: string;
    personaFacultadData?: string;
  }>();

  const isEditing = !!params.personaFacultadId;
  let initialData: DisplayPersonaFacultadFromList | null = null;
  if (isEditing && params.personaFacultadData) {
    try {
      initialData = JSON.parse(
        params.personaFacultadData
      ) as DisplayPersonaFacultadFromList;
    } catch (e) {
      console.error("Error parsing personaFacultadData:", e);
    }
  }
  ...
  const handleSave = async () => {
    ...
    const payload: ApiPersonaFacultad = {
    ...
    try {
      let response;
      let responseData: ApiPersonaFacultad;

      if (isEditing && initialData?.personafacultadid) {
        response = await fetch(
          `http://localhost:8000/personafacultad/${initialData.personafacultadid}`,
          {
            method: "PUT",
        ...
        response = await fetch("http://localhost:8000/personafacultad", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
      ...
      router.replace({
        pathname: "/personafacultad",
        params: isEditing
          ? { updatedPersonaFacultad: JSON.stringify(responseData) }
          : { newPersonaFacultad: JSON.stringify(responseData) },
      });
    } catch (e: any) {
